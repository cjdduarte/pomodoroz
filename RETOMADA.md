...
- Correcoes pos-review locais: janelas de semana/mes alinhadas a dias locais, fluxo diario em ordem cronologica para periodos fixos, topo do Relatorio compactado para comecar no filtro e nas metricas, atualizacao periodica do "agora" na tela aberta, remocao das chaves i18n mortas `byTaskList`, `periodReport` e `clearConfirm`, remocao dos artefatos `.playwright-mcp/*` e `statistics-period-progress.png`, e ignore de `.playwright-mcp/`.
