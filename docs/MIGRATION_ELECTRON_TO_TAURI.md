...
| 3b — Flatten Structure            | In progress | 2026-04-17  | Kickoff completed (scripts, renderer `src` relocation, dependency consolidation, `shareables` operational decoupling, workspace/package removal, and path-based pnpm filters); remaining step is final monorepo collapse to single root package layout |
...
Current state: Phase 1 closed, Phase 2a/2b/2c/2d/2e validated (Linux dev runtime), Phase 2g/2h/2i manual parity checklists are closed, Linux operational script cycle (`validar-tudo` options 5/6 + `uninstall purge`) was revalidated on 2026-04-16, and Phase 2f resumed in 26.4.18 with updater wiring kickoff (`tauri-plugin-updater` + policy-sync checks), updater key/feed scaffolding (`pubkey` + `createUpdaterArtifacts`) and a manual signed-assets workflow, while install/restart hardening remains pending. Phase 3a is closed, and Phase 3b kickoff now includes root script decoupling from `lerna run`, renderer source relocation to root `src`, renderer dependency consolidation on the root manifest, operational decoupling from `@pomodoroz/shareables` in scripts/workflow/Electron runtime imports, removal of the legacy `app/shareables` workspace package, and path-based `pnpm` filters (`./app/renderer`, `./app/electron`) in root/scripts/workflows.
...
- [x] Validate manual parity path for open-at-login behavior (enable/disable persistence + startup behavior) without regressions in timer/tray/notification flows.
...
- [x] Validate manual parity path for native task transfer dialogs (export save path selection, import file picker, invalid JSON handling).

Phase 2i kickoff snapshot (2026-04-15):
...
- Progress snapshot (2026-04-17):
  - [x] Root operational scripts migrated from `lerna run` to `pnpm -r --filter` (`dev:*`, `build*`, `release*`, `clean`), with filter targeting switched from workspace names to paths (`./app/renderer`, `./app/electron`).
