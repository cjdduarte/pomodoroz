...
| 3b — Flatten Structure            | In progress | 2026-04-17  | Root-driven flow is active (no `pnpm-workspace.yaml`, no renderer workspace manifest, no `pnpm --filter` in operational wrappers); remaining step is removing the Electron packaging manifest (`app/electron/package.json`) after migrating its build metadata safely |
...
Current state: Phase 1 closed, Phase 2a/2b/2c/2d/2e validated (Linux dev runtime), Phase 2g/2h/2i manual parity checklists are closed, Linux operational script cycle (`validar-tudo` options 5/6 + `uninstall purge`) was revalidated on 2026-04-16, and Phase 2f resumed in 26.4.18 with updater wiring kickoff (`tauri-plugin-updater` + policy-sync checks), updater key/feed scaffolding (`pubkey` + `createUpdaterArtifacts`) and a manual signed-assets workflow, while install/restart hardening remains pending. Phase 3a is closed, and Phase 3b now includes root script decoupling from `lerna run`, renderer source relocation to root `src`, renderer dependency consolidation on the root manifest, operational decoupling from `@pomodoroz/shareables` in scripts/workflow/Electron runtime imports, removal of the legacy `app/shareables` package, removal of `pnpm-workspace.yaml`, removal of `app/renderer/package.json`, and root-driven wrappers/workflows without `pnpm --filter` workspaces.
